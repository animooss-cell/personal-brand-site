...
import { DownloadResourceType } from "@/lib/types";
...
export default async function AdminDownloadsPage() {
  ...
  const { data: downloads } = await supabase
    .from("downloads")
    .select("id, title, slug, category, resource_type, published, updated_at")
    .order("updated_at", { ascending: false });

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">منابع دانلودی</h1>
        <Link
            ...
            {downloads?.map((item) => (
              <tr key={item.id} className="border-t border-gray-100">
                ...
                <td className="px-5 py-3 text-slate-500">
                  {resourceTypeLabel(item.resource_type as DownloadResourceType)}
                </td>
